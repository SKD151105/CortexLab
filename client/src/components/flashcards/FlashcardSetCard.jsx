...
const FlashcardSetCard = ({ flashcardSet }) => {
  ...
  return (
    <div
      className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-indigo-300 rounded-2xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-indigo-500/10 flex flex-col justify-between"
      onClick={handleStudyNow}
    >
      <div className="space-y-4">
        {/* Icon and Title */}
        <div className="flex items-start gap-4">
          <div className="shrink-0 w-12 h-12 rounded-xl bg-linear-to-br from-indigo-100 to-violet-100 flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
              Created {formatRelativeTime(flashcardSet.createdAt)}
            </p>
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-100">
        <button
          ...
          className="group/btn relative w-full h-11 bg-linear-to-r from-indigo-50 to-violet-100 hover:from-indigo-600 hover:to-violet-600 text-indigo-700 hover:text-white font-semibold text-sm rounded-xl transition-all duration-200 active:scale-95 overflow-hidden"
        >
